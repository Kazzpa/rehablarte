...
from models.chat_entity import ReChatSession, ReChat, RePrivateChat, ReChatGroup, ReChatMember
...
async def get_member_from_chat(chat: ReChatGroup, idOrUsername: str) -> ReChatMember | None:
    logger.info("Searching member in session")
    member_list = chat.chat_members
    return next(filter(lambda member: member.id == idOrUsername | member.username == idOrUsername, member_list))
...
async def initialize_session(state: FSMContext, chat: Chat):
    ...
    if chat_type is None:
        ...
        new_chat = RePrivateChat(id=chat_id, firstInteraction=datetime.now().date(), username=username)
    elif chat_type in ["group", "supergroup"]:
        logger.info("Initializing session as group chat")
        new_chat = ReChatGroup(id=chat_id, firstInteraction=datetime.now().date(), chat_members=[], type=chat_type)
